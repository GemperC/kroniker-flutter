...
class _HomePageState extends State<HomePage> {
  ...
  //   });
  // }
  void _getUserDocDataFromFirestore(UserRecord userRecord) async {
    List<String> _imageUrls = [];
    List<String> _gameIdList = [];

    final QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        .collection('games')
        .where(FieldPath.documentId, whereIn: userRecord.myGames)
        .get();
    ...
    for (var document in querySnapshot.docs) {
      ...
      if (data != null && data.containsKey('id')) {
        String gameId = data['id'];
        _gameIdList.add(gameId);
      }
    }

    setState(() {
      imageBannersUrls = _imageUrls;
      gameIdList = _gameIdList;
    });
  }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
          body: FutureBuilder(
              future: userRecordServices.getUserData(user.uid),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  if (snapshot.hasError) {
                    ...
                    final userDocData = snapshot.data!;
                    return Stack(
                      children: [
                        Column(
                                                              ...
                                                              onTap: () {
                                                                Navigator.push(
                                                                    ...
                                                                    MaterialPageRoute(
                                                                        builder:
                                                                            (context) =>
                                                                                GameMenuPage(gameData: myGames[index])));
                                                              },
                                                              child: Container(
                                                                height: 30,
                                                                width: 80,
